...
public class TemplateController {
    ...
    public ResponseEntity createTemplate(@RequestBody @Valid TemplateDto templateDto, Errors errors) {
        ...
        return ResponseEntity.created(linkTo(TemplateController.class).slash(createdMailTemplate.getId()).toUri()).body(DefaultResponse.builder().status(ResponseCode.OK).data(createdMailTemplate).build());
    }
    @GetMapping("/{id}")
